...
class SjhaskdoioasdkController extends ControllerBase{
     ...
     * 整天执行
     */
    public function shell4Action(){
        try{
            $dateTime   = \Util\common::getDate(); //当天日期
            $currencyId = 4; //币种 1  //此币种只针对两个测试账户，其他用户只能看不能操作。
            $whereShell['date'] = $dateTime;
            $whereShell['currency_id'] = $currencyId;
            $shellInfo = ExShell::findRow($whereShell);
            if($shellInfo){
     ...
     * 整天执行
     */
    public function shell6Action(){
        try{
            $dateTime   = \Util\common::getDate(); //当天日期
            $currencyId = 6; //币种 1  //此币种只针对两个测试账户，其他用户只能看不能操作。
            $whereShell['date'] = $dateTime;
            $whereShell['currency_id'] = $currencyId;
            $shellInfo = ExShell::findRow($whereShell);
            if($shellInfo){
     ...
     * 整天执行
     */
    public function shell7Action(){
        try{
            $dateTime   = \Util\common::getDate(); //当天日期
            $currencyId = 7; //币种 1  //此币种只针对两个测试账户，其他用户只能看不能操作。
            $whereShell['date'] = $dateTime;
            $whereShell['currency_id'] = $currencyId;
            $shellInfo = ExShell::findRow($whereShell);
            if($shellInfo){
     ...
     * 整天执行
     */
    public function shell9Action(){
        try{
            $dateTime   = \Util\common::getDate(); //当天日期
            $currencyId = 9; //币种 1  //此币种只针对两个测试账户，其他用户只能看不能操作。
            $whereShell['date'] = $dateTime;
            $whereShell['currency_id'] = $currencyId;
            $shellInfo = ExShell::findRow($whereShell);
            if($shellInfo){
