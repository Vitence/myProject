  ...
  $.ajax({
    ...
    success:function(result){
      ...
      for(var i = 0 ; i < result.data.length ; i++){
        resultData[i] = [result.data[i].date,Number(result.data[i].open_price),Number(result.data[i].close_price),Number(result.data[i].min),Number(result.data[i].max),parseInt(Math.random()*10000), 0, 0.11, -0.28, -0.34];
      }
...
var twdhqdata = splitData(getKData(1));
var myjfdata = splitData(getKData(2));
